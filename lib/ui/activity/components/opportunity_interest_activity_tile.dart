...
class OpportunityInterestActivityTile extends StatelessWidget {
  const OpportunityInterestActivityTile({
    required this.activity,
    super.key,
  });

  final OpportunityInterest activity;

  Future<void> onClick(BuildContext context) async {
    final nav = context.read<NavigationBloc>();

    final database = context.read<DatabaseRepository>();
    final loop = await database.getLoopById(activity.loopId);
    if (loop.isNone) {
      return;
    }
    nav.push(LoopPage(loop: loop.unwrap, loopUser: const None()));
  }
                        ...
                        children: [
                          GestureDetector(
                            ...
                            child: ListTile(
                              ...
                              subtitle: Text(
                                'is interested in your opportunity 📩',
                                style: TextStyle(
                                  fontWeight:
                                      markedRead ? null : FontWeight.bold,
                                ),
